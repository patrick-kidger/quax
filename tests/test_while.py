...
def test_while_basic():
    a = Unitful(jnp.asarray(1.0), meters)
    b = Unitful(jnp.asarray(2.0), meters)
    c = Unitful(jnp.asarray(10.0), meters)
...
def test_while_different_units():
    a = Unitful(jnp.asarray([1.0]), meters)
    b = Unitful(jnp.asarray([2.0]), meters)
    c = Unitful(jnp.asarray([10.0]), kilograms)
    try:
...
def test_while_grad_closure():
    ...
    c = Unitful(jnp.asarray(10.0), meters)
    dummy = Unitful(jnp.asarray(1.0), meters)
